...
class AIService:
    ...
    def _call_cursor(self, prompt: str) -> Dict[str, Any]:
        """Call Cursor API (OpenAI-compatible)
        
        Note: Cursor API endpoints may vary. Common options:
        - https://api.cursor.sh/v1
        - https://api.cursor.com/v1
        - Or use OpenAI API directly (https://api.openai.com/v1) if Cursor keys work there
        """
        try:
            ...
            for endpoint in endpoints_to_try:
                try:
                    ...
                    response = client.chat.completions.create(
                        model=self.model.model_id,
                        messages=[
                            {"role": "system", "content": "You are a helpful software engineering assistant specialized in debugging and code fixes."},
                            {"role": "user", "content": prompt}
                        ],
                        temperature=0.7,
                        max_tokens=2000,
                    ...
                    # If successful, log which endpoint worked
                    if endpoint != base_url:
                        logger.info(f"Cursor API works with endpoint: {endpoint}")
                    
                    return {
                        'suggestion': content,
                        'explanation': "Generated by Cursor",
                        'confidence': 'high',
                        'model': self.model.model_id,
                        'provider': 'cursor',
                    }
                    ...
                    last_error = e
                    logger.debug(f"Cursor endpoint {endpoint} failed: {e}")
                    continue
            
            # If all endpoints failed, raise the last error
            if last_error:
                raise ValueError(
                    f"Cursor API failed on all endpoints. Last error: {last_error}. "
                    f"Please verify your API key and base URL. "
                    f"Tried endpoints: {', '.join(endpoints_to_try)}. "
                    f"Note: Cursor API endpoint may have changed. Check Cursor documentation for the correct endpoint."
                )
            else:
                raise ValueError("No Cursor API endpoints available to try")
                
        except Exception as e:
            logger.error(f"Error calling Cursor: {e}")
            raise
